...
{
    [Serializable]
    public class LevelGroupConfig : GroupConfig<GroupDataMinMax<long, int>, long, int>
    {
        public override void ApplyRemoteConfig(string json)
        {
            ...
            try
            {
                ...
                foreach (var group in jsonData.AsArray)
                {
                    ...
                    {
                        Debug.LogWarning($"[LevelGroupConfig] Invalid group format: {group.Value}");
                    }
            ...
            {
                Debug.LogError($"[LevelGroupConfig] Failed to apply remote config: {e.Message}");
            }
        }

        public override int GetGroupId(long level)
        {
            if (level <= 0 || Groups.Count == 0)
                return 0;

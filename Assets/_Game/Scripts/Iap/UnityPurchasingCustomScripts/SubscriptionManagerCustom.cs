...
namespace UnityEngine.Purchasing.Custom
{
    /// <summary>
    ...
    public class SubscriptionManagerCustom
    {
        ...
        /// <param name="developerPayload">Carried-over metadata from prior call to <typeparamref name="SubscriptionManagerCustom.UpdateSubscription"/> </param>
        ...
        public static void UpdateSubscription(Product newProduct, Product oldProduct, string developerPayload,
            Action<Product, string> appleStore, Action<string, string> googleStore)
        {
            if (oldProduct.receipt == null)
            {
                Debug.LogError(
                    "The product has not been purchased, a subscription can only be upgrade/downgrade when has already been purchased");
                return;
            }

            var receipt_wrapper = (Dictionary<string, object>)MiniJson.JsonDecode(oldProduct.receipt);
            if (receipt_wrapper == null || !receipt_wrapper.ContainsKey("Store") ||
                !receipt_wrapper.ContainsKey("Payload"))
            {
            ...
            if (payload != null)
            {
                switch (store)
                {
                    case "GooglePlay":
                    {
                        var oldSubscriptionManager = new SubscriptionManagerCustom(oldProduct, null);
                        SubscriptionInfoCustom oldSubscriptionInfo;
                        try
                        {
                        ...
                        catch (Exception e)
                        {
                            Debug.unityLogger.LogError(
                                "Error: the product that will be updated does not have a valid receipt", e);
                            return;
                        }
        ...
        public static void UpdateSubscriptionInGooglePlayStore(Product oldProduct, Product newProduct,
        ...
        public static void UpdateSubscriptionInAppleStore(Product newProduct, string developerPayload,
        ...
        public SubscriptionManagerCustom(Product product, string intro_json)
        {
        ...
        /// <param name="intro_json">From <typeparamref name="IAppleExtensions.GetIntroductoryPriceDictionary"/></param>
        public SubscriptionManagerCustom(string receipt, string id, string intro_json)
        {
        ...
        /// <summary>
        /// Convert my Product into a <typeparamref name="SubscriptionInfoCustom"/>.
        ...
        public SubscriptionInfoCustom getSubscriptionInfo()
        {
            if (receipt != null)
            {
                ...
                if (validPayload && validStore)
                {
                    ...
                    switch (store)
                    {
                        ...
                        default:
                        {
                            Debug.LogError($"Store not supported: {store}");
                            return null;
                        }
                    }
                }
        ...
        private SubscriptionInfoCustom getAmazonAppStoreSubInfo(string productId)
        {
            return new SubscriptionInfoCustom(productId);
        }

        private SubscriptionInfoCustom getAppleAppStoreSubInfo(string payload, string productId)
        {
            ...
            return inAppPurchaseReceipts.Count == 0
                ? null
                : new SubscriptionInfoCustom(findMostRecentReceipt(inAppPurchaseReceipts), intro_json);
        }
        ...
        private SubscriptionInfoCustom getGooglePlayStoreSubInfo(string payload)
        {
            ...
            return new SubscriptionInfoCustom(skuDetail, isAutoRenewing, purchaseDate, isFreeTrial,
    ...
    /// </summary>
    /// <seealso cref="SubscriptionManagerCustom.getSubscriptionInfo"/>
    public class SubscriptionInfoCustom
    {
        ...
        public SubscriptionInfoCustom(AppleInAppPurchaseReceipt r, string intro_json)
        {
            var productType =
                (AppleStoreProductType)Enum.Parse(typeof(AppleStoreProductType), r.productType.ToString());
            ...
            if (!string.IsNullOrEmpty(intro_json))
            {
                ...
                introductory_price = intro_wrapper.TryGetString("introductoryPrice") +
                                     intro_wrapper.TryGetString("introductoryPriceLocale");
                ...
                else
                {
                    try
                    {
                        introductory_price_cycles =
                            Convert.ToInt64(intro_wrapper.TryGetString("introductoryPriceNumberOfPeriods"));
                    ...
                    catch (Exception e)
                    {
                        Debug.unityLogger.Log(
                            "Unable to parse introductory period cycles and duration, this product does not have configuration of introductory price period",
                            e);
            ...
            var current_date = DateTimeManager.Now;
            ...
            {
                is_cancelled = (r.cancellationDate.Ticks > 0) && (r.cancellationDate.Ticks < current_date.Ticks)
                    ? Result.True
                    : Result.False;
                is_subscribed = r.subscriptionExpirationDate.Ticks >= current_date.Ticks ? Result.True : Result.False;
                is_expired =
                    (r.subscriptionExpirationDate.Ticks > 0 && r.subscriptionExpirationDate.Ticks < current_date.Ticks)
                        ? Result.True
                        : Result.False;
                is_free_trial = (r.isFreeTrial == 1) ? Result.True : Result.False;
                is_auto_renewing = ((productType == AppleStoreProductType.AutoRenewingSubscription) &&
                                    is_cancelled == Result.False
            ...
            remainedTime = is_subscribed == Result.True
                ? r.subscriptionExpirationDate.Subtract(current_date)
                : TimeSpan.Zero;
        }
        ...
        public SubscriptionInfoCustom(string skuDetails, bool isAutoRenewing, DateTime purchaseDate, bool isFreeTrial,
            bool hasIntroductoryPriceTrial, bool purchaseHistorySupported, string updateMetadata)
        {
            ...
            {
                introductory_price_period = introductory_price_period_string != null &&
                                            introductory_price_period_string.Equals(sub_period)
                    ? subscriptionPeriod
                ...
                total_introductory_duration = accumulateIntroductoryDuration(
                    parsePeriodTimeSpanUnits(introductory_price_period_string), this.introductory_price_cycles);
            }

            // if this subscription is updated from other subscription, the remaining time will be applied to this subscription
            var extra_time = TimeSpan.FromSeconds(updateMetadata == null
                ? 0.0
                : computeExtraTime(updateMetadata, subscriptionPeriod.TotalSeconds));

            var time_since_purchased = DateTimeManager.Now.Subtract(purchaseDate);
            ...
            else if (time_since_purchased < freeTrialPeriod.Add(extra_time).Add(total_introductory_duration))
            {
                ...
                subscriptionExpireDate = nextBillingDate(introductory_price_begin_date,
                    parsePeriodTimeSpanUnits(introductory_price_period_string));
            }
            else
            {
                // no matter sub is cancelled or not, the expire date will be next billing date
                var billing_begin_date =
                    this.purchaseDate.Add(freeTrialPeriod.Add(extra_time).Add(total_introductory_duration));
                subscriptionExpireDate = nextBillingDate(billing_begin_date, parsePeriodTimeSpanUnits(sub_period));
            }

            remainedTime = subscriptionExpireDate.Subtract(DateTimeManager.Now);
        ...
        public SubscriptionInfoCustom(string productId)
        {
        ...
        /// <seealso cref="DateTimeManager.Now"/>
        ...
        public void PrintDebug(string prefix = "")
        {
            ...
            Debug.Log(prefix + "now: " + DateTimeManager.Now);
            Debug.Log(prefix + "productId: " + productId);
            Debug.Log(prefix + "purchaseDate: " + purchaseDate);
            Debug.Log(prefix + "subscriptionExpireDate: " + subscriptionExpireDate);
            Debug.Log(prefix + "subscriptionCancelDate: " + subscriptionCancelDate);
            Debug.Log(prefix + "remainedTime: " + remainedTime);
            Debug.Log(prefix + "introductory_price: " + introductory_price);
            Debug.Log(prefix + "introductory_price_period: " + introductory_price_period);
            Debug.Log(prefix + "introductory_price_cycles: " + introductory_price_cycles);
            Debug.Log(prefix + "freeTrialPeriod: " + freeTrialPeriod);
            Debug.Log(prefix + "subscriptionPeriod: " + subscriptionPeriod);
            Debug.Log(prefix + "free_trial_period_string: " + free_trial_period_string);
            Debug.Log(prefix + "is_auto_renewing: " + is_auto_renewing);
            Debug.Log(prefix + "sku_details: " + sku_details);
            Debug.Log("---------------------------SubscriptionInfo------------------------------");
        }
        ...
        private DateTime nextBillingDate(DateTime billing_begin_date, TimeSpanUnits units)
        {
            ...
            while (DateTime.Compare(next_billing_date, DateTimeManager.Now) <= 0)
            {
        ...
        private double computeExtraTime(string metadata, double new_sku_period_in_seconds)
        {
            ...
            var result = old_sku_remaining_seconds / (double)old_sku_period_in_seconds * old_sku_price_in_micros /
                new_sku_price_in_micros * new_sku_period_in_seconds;
            return result;
        }

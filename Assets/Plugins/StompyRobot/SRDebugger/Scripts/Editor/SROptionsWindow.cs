    ...
    class SROptionsWindow : EditorWindow
    {
        ...
        void PopulateTypeLookup()
        {
            _typeLookup = new Dictionary<Type, Action<OptionDefinition>>()
            {
                ...
                {typeof(SRDropDownData), OnGUI_DropDown},
            };
        }
        ...
        void OnGUI_DropDown(OptionDefinition op)
        {
            if (op.Property.PropertyType != typeof(SRDropDownData))
            {
                Debug.LogError("Invalid property type for dropdown control: " + op.Property.PropertyType);
                return;
            }
            
            var dropDownData = (SRDropDownData) op.Property.GetValue();
            
            EditorGUI.BeginChangeCheck();
            var newValue = EditorGUILayout.Popup(op.Name, dropDownData.SelectionIndex, dropDownData.Options.Select(c => c.ToString()).ToArray());

            if (EditorGUI.EndChangeCheck())
            {
                dropDownData.SelectionIndex = newValue;
                op.Property.SetValue(dropDownData);
            }
        }
